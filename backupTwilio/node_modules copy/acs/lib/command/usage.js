...
exports.run = function(args, opts) {
    ...
    console.log('Retrieving usage data from the server');
    var name = null;
    ...
    var parseResult = function(err, unzippedBody) {
        ...
        if( result.errcode && result.errcode === 100 ) {
            ...
            **/
            ...
            console.log();
            var len = result.stats.length, stat, fmt;
            for( var i = 0; i < len; i++ ){
                
                stat = result.stats[i];

                console.log(((stat.date? moment(stat.date).format() + ' | ' : '')) + '\t'
                + (opts.show_serverid?(('[Server:' + (stat.serverid===undefined?'Unassigned]\t\t':stat.serverid+']')).yellow + '\t'):'')
                + 'memory: { free: ' + (stat.process.memory.free/1048576).toFixed(2) + 'M total: ' + parseInt(stat.process.memory.total/1048576) + 'M }\t'
                + 'loadavg: [ ' + (stat.os.load*100).toFixed(2) + '% ]');
            }

            if(stat) {
                u.saveLastLogTime(LOG_TYPE, startLogDateTimeStamp, endLogDateTimeStamp, moment(stat.date).format());
                console.log();
            } else {
                console.log('No usage log found.\n');
            }
        }
    };
